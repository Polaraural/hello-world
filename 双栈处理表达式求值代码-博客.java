...
public class Calculator {
      ...
      public static void main(String[] args) {
            ...
            Scanner scanner = new Scanner(System.in);//输入测试样例组数
            String input = "";//回车缓冲
            final String CLOSE_MARK = "bye";
            System.out.println("input an expression:");//输入表达式
            input = scanner.nextLine();//输入后换行
            while (input.length() != 0 && !CLOSE_MARK.equals((input))) { //CLOSE_MARK是什么鬼
      ...
      private static void toPolishNotation(String input)
                  throws IllegalArgumentException, NumberFormatException { //这个throw又是什么鬼啊
            int len = input.length();
            char c, tempChar;
            Stack<Character> s1 = new Stack<Character>();  //存放操作符号的栈
            Stack<Double> s2 = new Stack<Double>();  //存放数字的栈
            Stack<Object> expression = new Stack<Object>();  //存放表达式  注意是Object 
            double number;
            int lastIndex = -1;
            for (int i=len-1; i>=0; --i) {
                  c = input.charAt(i);// CharAt又是什么鬼啊 
                  if (Character.isDigit(c)) { //判断是否是数字
                        lastIndex = readDoubleReverse(input, i); //readDoubleReverse(input,i)
                        number = Double.parseDouble(input.substring(lastIndex, i+1));//paeseDouble
                        s2.push(number);
                        i = lastIndex; 
                        ...
                        while (!s1.isEmpty()
                                    && s1.peek() != ')'
                                    && priorityCompare(c, s1.peek()) < 0) {  //priorityCompare
                              expression.push(s1.peek());
                              s2.push(calc(s2.pop(), s2.pop(), s1.pop()));  //calc
                        }
                        s1.push(c);
                  } else if (c == ')') {
                        s1.push(c);
